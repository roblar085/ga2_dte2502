...
from AdvantageActorCriticAgentModel import AdvantageActorCriticAgentModel
from SupervisedLearningAgentModel    import SupervisedLearningAgentModule
...
from DeepQLearningAgentModel import DeepQLearningAgentModel
from PolicyGradientAgentModel import PolicyGradientAgentModel
...
class DeepQLearningAgent(Agent):
    ...
    def _get_model_outputs(self, board, model=None):
        ...
        if isinstance(res, tuple):
            ...
            return model1.numpy(),model2.numpy()
    ...
    def move(self, board, legal_moves, value=None):
        ...
        res = self._get_model_outputs(board, self._model)
        if isinstance(res,tuple):
          model1,model2 = res
          return np.argmax(np.where(legal_moves == 1, model2, -np.inf), axis=1)
        else:
          return np.argmax(np.where(legal_moves == 1, res, -np.inf), axis=1)
    ...
    def _agent_model(self):
        ...
        model = DeepQLearningAgentModel(self._board_size, self._n_frames, self._n_actions,m)
    ...
    def get_action_proba(self, board, values=None):
        ...
        model1 = model1 / model2.sum(axis=1).reshape((-1, 1))
    ...
    def train_agent(self, batch_size=32, num_games=1, reward_clip=False):
        ...
        target_torch = torch.tensor(target, dtype=torch.float32).to(device)
...
class PolicyGradientAgent(DeepQLearningAgent):
    ...
    def _agent_model(self):
        ...
        """
        input_board = PolicyGradientAgentModel(self._board_size, self._n_frames, self._n_actions,0.01)
    ...
    def train_agent(self, batch_size=32, beta=0.1, normalize_rewards=False,
        ...
        s_prepeared = self._prepare_input(s)
        s_torch = torch.tensor(s_prepeared, dtype=torch.float32).to(device)
        target_torch = torch.tensor(target, dtype=torch.float32).to(device)
        self.loss = torch.cnn.CrossEntropyLoss(beta)
        loss = self.loss(torch.Tensor(self._model(s_torch)), target_torch)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
...
class AdvantageActorCriticAgent(PolicyGradientAgent):
    ...
    def __init__(self, board_size=10, frames=4, buffer_size=10000,
        ...
        self.model_values._optimizer =  torch.optim.RMSprop(self.model_values.parameters(), lr=5e-4)
        self.optimizer =  self.model_full._optimizer
    def _agent_model(self):
        ...
        model_logits = AdvantageActorCriticAgentModel(self._board_size, self._n_frames, self._n_actions,outputs=['action_logits'])
        model_full = AdvantageActorCriticAgentModel(self._board_size, self._n_frames, self._n_actions,outputs=['action_logits','state_values'])
        model_values = AdvantageActorCriticAgentModel(self._board_size, self._n_frames, self._n_actions,outputs=['state_values'])
    ...
    def train_agent(self, batch_size=32, beta=0.001, normalize_rewards=False,
        ...
        # calculate V values
        if (self._use_target_net):
            next_s_pred = self._target_net(torch.Tensor(next_s_prepared)).cpu().detach().numpy()
        else:
            next_s_pred = self._values_model(torch.Tensor(next_s_prepared)).cpu().detach().numpy()


        s_pred = self._values_model(torch.Tensor(s_prepared)).cpu().detach().numpy()
        ...
        model1,model2 = model(torch.Tensor(s_prepared))
        model1,model2 = model1.cpu(),model2.cpu()
        policy = torch.nn.functional.softmax(model1)
        log_policy = torch.nn.functional.log_softmax(model1)
        # calculate loss
        ...
        critic_loss = mean_huber_loss(torch.Tensor(critic_target), model2)
...
class SupervisedLearningAgent(DeepQLearningAgent):
    ...
    def __init__(self, board_size=10, frames=2, buffer_size=10000,
        ...
        self._model_action = SupervisedLearningAgentModule(inputs=self._model.get_layer('input').input, outputs=self._model_action_out)
        self.optimizer = optim.Adam(self._model_action.parameters(), lr=0.0005)
